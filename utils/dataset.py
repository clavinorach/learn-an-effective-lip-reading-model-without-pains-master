...
class idev1Dataset(Dataset):
    def __init__(self, phase, args):
        ...
        for (i, label) in enumerate(self.labels):
            files = glob.glob(os.path.join('idev1_roi_80_116_175_211_npy_gray_pkl_jpeg', label, phase, '*.pkl'))                    
            files = sorted(files)
    ...
    def __getitem__(self, idx):
        ...
        tensor = torch.load(self.list[idx], map_location='cpu', weights_only=False)                    
